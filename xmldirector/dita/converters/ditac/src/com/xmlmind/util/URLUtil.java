/*
 * Copyright (c) 2002-2015 Pixware SARL. All rights reserved.
 *
...
public final class URLUtil {
     ...
     * 
     * @param ancestorURL a hierachical or "jar:" URL
     * @param url a hierachical or "jar:" URL
     * @return <code>true</code> if <tt>ancestorURL</tt> is equal to 
     * or is an ancestor directory of <tt>url</tt>; 
     * <code>false</code> otherwise.
     */
    public static boolean isAncestorOf(URL ancestorURL, URL url) {
        if (!sameRoot(ancestorURL, url)) {
            return false;
        }

        String[] path1 = splitPath(ancestorURL);
        if (path1 == null) {
            return false;
        }

        String[] path2 = splitPath(url);
        if (path2 == null) {
            return false;
        }
        ...
        // Assumes that paths have been canonicalized, that is, ".", "..",
        // "//", etc, have been removed.
        return path2[1].startsWith(path1[1]);
    }
    ...
    /**
     * Same as {@link #toDisplayForm} but <tt>file:</tt> URLs 
     ...
     * 
     * @param url a hierarchical URL possibly having a fragment 
     * and a query string
    ...
    public static boolean exists(URL url) {
        ...
        try {
            ...
            if (connection instanceof HttpURLConnection) {
                ...
                try {
                    in.read(); // Try to read on byte; empty file returns -1.
                } finally {
    ...
    public static long lastModified(URL url) {
        ...
        try {
            ...
            date = connection.getLastModified(); // 0 if unknown
        } catch (IOException ignored) {}
    ...
    /**
     * Checks that specified HTTP connection returned "200 OK". No effect
     * when specified connection is not a <code>HttpURLConnection</code> 
     * (e.g. it's a <code>JarURLConnection</code>).
     *
     * @exception IOException when specified HTTP connection returned
     * a code other than "200 OK". The message of the exception contains
     * information about what happened.
     */
    public static void checkHTTPConnection(URLConnection connection) 
        throws IOException {
        if (connection instanceof HttpURLConnection) {

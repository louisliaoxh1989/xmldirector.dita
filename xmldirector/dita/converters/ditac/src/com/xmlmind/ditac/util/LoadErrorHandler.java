/*
 * Copyright (c) 2015 Pixware SARL. All rights reserved.
 *
...
/*package*/ final class LoadErrorHandler implements ErrorHandler {
    ...
    private static final String ELEMENT_TYPE_ERROR_HEAD="Element type \"";
    private static final String ELEMENT_TYPE_ERROR_TAIL="\" must be declared.";
    ...
    public void error(SAXParseException e)
        ...
        if (message != null &&
            message.startsWith(ELEMENT_TYPE_ERROR_HEAD) && 
            message.endsWith(ELEMENT_TYPE_ERROR_TAIL)) {
            int start = ELEMENT_TYPE_ERROR_HEAD.length();
            int end = message.indexOf('"', start);
            if (end > start) {
                String elementType = message.substring(start, end);
                int colon = elementType.indexOf(':');
                if (colon > 0 && colon < elementType.length()-1) {
                    // Assume that it's an svg:svg or a mml:math found
                    // inside a foreign element.
                    warning(e);
                    return;
                }
            }

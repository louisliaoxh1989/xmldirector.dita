...
public class Converter {
    ...
    public boolean registerXEP(String exeFileName) {
        ...
        // By default PDF_VERSION is 1.3. We need 1.5 to support
        // rx:media-object. But there is no way to specify this using a
        // command-line option. (Of course, adding -DPDF_VERSION=1.5 below
        // will not work.) You'll have to add 
        // <option name="PDF_VERSION" value="1.5"/> 
        // to 
        // <generator-options format="PDF">
        // found in $XEP_HOME/xep.xml for that.

        // -valid is a workaround for XEP bug: renderx #22766

        StringBuilder buffer = new StringBuilder();
        buffer.append('"');
        buffer.append(exeFile);
        buffer.append("\" -quiet -valid -fo \"%I\" -ps \"%O\"");
        ...
        buffer.append('"');
        buffer.append(exeFile);
        buffer.append("\" -quiet -valid -fo \"%I\" -pdf \"%O\"");
    ...
    public int run(String[] args) {
        ...
        // The actual main language of the document. May be null.
        docLang = preProc.getDocLang();
    ...
    private boolean convertToHTMLForm(File[] preProcFiles, 
        ...
        case XHTML1_1:
            ...
            break;
        case XHTML5:
            styleSheetPath = "xhtml/xhtml5.xsl";
            break;
        case HTML:
    ...
    private boolean convertToWebHelp(File[] preProcFiles, 
        ...
        switch (format) {
        case WEB_HELP5:
            styleSheetPath = "webhelp/webhelp5.xsl";
            break;
        case WEB_HELP:
        default:
            styleSheetPath = "webhelp/webhelp.xsl";
            break;
        }
        URL transformURL = getStyleSheet(styleSheetPath);
        if (transformURL == null) {
        ...
        if (docLang != null) {
            ...
            params2 = ArrayUtil.append(params2, docLang);
        }
    ...
    private File[] preProcessEPUB(URL[] inFiles, File outFile) 
        ...
        File outFile2 = 
            new File(preProcTempDir, 
                     (format == Format.EPUB3)? "_.xhtml" : "_.html");
        return preProc.process(inFiles, outFile2);
    }
    ...
    private boolean convertToEPUB(File[] preProcFiles, String outExtension) {
        ...
        switch (format) {
        case EPUB3:
            styleSheetPath = "epub/epub3.xsl";
            htmlExt = "xhtml";
            break;
        default:
            styleSheetPath = "epub/epub.xsl";
            htmlExt = "html";
            break;
        }
        URL transformURL = getStyleSheet(styleSheetPath);
    ...
    private boolean parseOptions(String[] args) {
        ...
        for (l = 0; l < args.length; ++l) {
            ...
            } else if ("-r".equals(arg) || "-resources".equals(arg) ||
                       "-i".equals(arg) || "-images".equals(arg)) {
                if (l+1 >= args.length) {
                    usage(null);
                    return false;
                }

                if (!setResourceHandler(
                      "com.xmlmind.ditac.convert.ResourceCopier", args[++l])) {
                    return false;
                }
            } else if ("-resourcehandler".equals(arg)) {
                if (l+2 >= args.length) {
                    usage(null);
                    return false;
                }

                if (!setResourceHandler(args[l+1], args[l+2])) {
                    return false;
                }
                l += 2;
    ...
    private boolean setResourceHandler(String className, String parameters) {
        try {
            ResourceHandler resourceHandler = 
                (ResourceHandler) Class.forName(className).newInstance();

            resourceHandler.parseParameters(parameters);

            preProc.setResourceHandler(resourceHandler);
            return true;
        } catch (Exception e) {
            usage(Msg.msg("cannotCreateResourceHandler", className, parameters,
                          ThrowableUtil.reason(e)));
            return false;
        }
    }
    ...
    private boolean setFilter(String location) {
        ...
        try {
            preProc.setFilter(new Filter(url, validate, console));
            return true;
        } catch (Exception e) {
    ...
    public static void main(String[] args) {
        ...
        autoRegisterFOConverters(converter);
    ...
    private static void autoRegisterFOConverters(Converter converter) {
        File installDir = AppUtil.findInstallDirectory();
        if (installDir == null) {
            return;
        }

        File plusDir = new File(installDir, "plus");
        if (!plusDir.isDirectory()) {
            return;
        }

        File[] files = plusDir.listFiles();
        ...
        for (File file : files) {
            ...
            if (baseName.startsWith("fop")) {
                File exeFile = new File(file, "fop");
                if (exeFile.isFile() && 
                    converter.registerFOP(exeFile.getPath())) {
                    break;
                }
            }
        }

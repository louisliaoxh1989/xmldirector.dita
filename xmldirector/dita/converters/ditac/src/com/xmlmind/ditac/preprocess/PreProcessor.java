...
public class PreProcessor implements Constants {
     ...
     */
    public static final String VERSION = "2.5.8_01";
    ...
    protected void configureFilter(Filter filter, File outDir) {
        ...
        filter.setResourceHandler(dryRun? null : resourceHandler);
        ...
        while (iter.hasNext()) {
            ...
            if (topicrefCount > 1) {
                ...
                for (int i = 1; i < topicrefCount; ++i) {
                    URL url2 = null;
                    try {
                        url2 = DITAUtil.doGetLocalTopicURL(topicrefs[i]);
                    } catch (IllegalArgumentException cannotHappen) {} 

                    if (url2 != null) {
                        URL url3 = duplicateTopics(url2, 
                                                   serialNum(url, serialNums), 
                                                   topicrefs[i]);

                        // Not toLabel(). Want to see the fragments.
                        console.verbose(Msg.msg("duplicatedTopics", 
                                                URLUtil.toDisplayForm(url2), 
                                                ((url3 == null)? "???" : 
                                                 URLUtil.toDisplayForm(url3))));
                    }
                }
            }
        }
    ...
    private static int serialNum(URL url, Map<URL,int[]> map) {
        if (url.getRef() != null) {
    ...
    protected URL duplicateTopics(URL url, int serialNum, Element topicref) {
        URL url2 = null;
        Document copy = null;
        ...
        if (loadedDoc != null) {
            String topicId = DITAUtil.getTopicId(url);
            if (topicId != null) {
                LoadedTopic loadedTopic = loadedDoc.findTopicById(topicId);
                if (loadedTopic != null) {
                    // Duplicate the topic ---

                    String rawBaseName2 = 
                        rawBaseNameOfCopy(url, serialNum, topicref);
                    ...
                    if (url2 != null) {
                        copy = DOMUtil.newDocument();

                        // Note that nested topics are copied too, which is
                        // what we want (e.g. for chunk=select-branch).
                ...
                String rawBaseName2 = rawBaseNameOfCopy(url, serialNum, 
                                                        topicref);
                ...
                if (url2 != null) {
                    copy = DOMUtil.newDocument();
                    copy.appendChild(
                      copy.importNode(loadedDoc.document.getDocumentElement(), 
                                      /*deep*/ true));
                }
            }
        }
        ...
        if (url2 != null) {
            LoadedDocument synthDoc = loadedDocs.put(url2, copy, 
                                                     /*process*/ false);
            synthDoc.setSynthetic(/*originalURL*/ loadedDoc.url);

            topicref.setAttributeNS(null, "href", url2.toExternalForm());
        }

        return url2;
    }

    private static String rawBaseNameOfCopy(URL url, int serialNum, 
                                            Element topicref) {
        String rawRootName = URLUtil.getRawBaseName(url);
        String rawExtension = URIComponent.getRawExtension(rawRootName);
        if (rawExtension != null) {
            rawRootName = URIComponent.setRawExtension(rawRootName, null);
        }
        ...
        String rawBaseName2 = quotePath(rootName2);
        if (rawExtension != null) {
    ...
    protected static final String quotePath(String segment) {
        String quoted = URIComponent.quotePath(segment);
        // quotePath does not replace e acute by "%c3%a9". 
        //
        // When an input filename contains accented characters, this causes 
        // u:currentChunk() to fail with the following error:
        // An empty sequence is not allowed as the result of 
        // function u:currentChunk().

        return URIComponent.encode(quoted);
    }
    ...
    protected String chunkBaseName(String rootName) {
        StringBuilder buffer = new StringBuilder();
        buffer.append(quotePath(rootName));
        if (extension != null) {
            buffer.append('.');
            buffer.append(quotePath(extension));
        }
        return buffer.toString();
    }
    ...
    protected boolean processObject(Element element, File outDir) {
        ...
        for (int i = 0; i < paramCount; ++i) {
            ...
            if ("source.src".equals(name)) {
                mimeType = null;
                if (i+1 < paramCount) {
                    Element nextParam = params[i+1];
                    name = DITAUtil.getNonEmptyAttribute(nextParam, 
                                                         null, "name");
                    if ("source.type".equals(name)) {
                        mimeType = DITAUtil.getNonEmptyAttribute(nextParam, 
                                                                 null, "value");
                    }
                }

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:attribute-set name="step-importance-optional">
    <xsl:attribute name="font-style">italic</xsl:attribute>
  </xsl:attribute-set>

  <xsl:attribute-set name="step-importance-required">
    ...
    <xsl:if test="$mark-important-steps eq 'yes'">
      ...
      <xsl:choose>
        <xsl:when test="$importance eq 'optional'">
          <fo:inline xsl:use-attribute-sets="step-importance-optional">
            <xsl:call-template name="localize">
        ...
        <xsl:when test="$importance eq 'required'">
          <fo:inline xsl:use-attribute-sets="step-importance-required">
            <xsl:call-template name="localize">
  ...
  <xsl:template match="*[contains(@class,' task/substeps ')]">
    <fo:list-block xsl:use-attribute-sets="substeps">
      <xsl:call-template name="xfcOLLabelFormat">
        <xsl:with-param name="olType" select="'a'"/>
      </xsl:call-template>
    ...
    <xsl:choose>
      <xsl:when test="u:isCompactSteps(parent::*)">
        <fo:list-item xsl:use-attribute-sets="compact-substep">
          <xsl:call-template name="orderedListItem">
            <xsl:with-param name="olType" select="'a'" tunnel="yes"/>
          </xsl:call-template>
        </fo:list-item>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:attribute-set name="choptionhd" use-attribute-sets="stentry">
  </xsl:attribute-set>
  ...
  <xsl:attribute-set name="chdeschd" use-attribute-sets="stentry">
  </xsl:attribute-set>

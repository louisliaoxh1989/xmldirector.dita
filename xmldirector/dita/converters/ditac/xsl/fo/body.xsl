...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="xfcOLLabelFormat">
    ...
    <xsl:if test="$foProcessor eq 'XFC'">
      ...
      <xsl:choose>
        <xsl:when test="$olStart ne ''">
          <xsl:attribute name="xfc:label-format" 
            select="concat('%{', $olType2, ';start=', $olStart, '}.')"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:attribute name="xfc:label-format" 
                         select="concat('%{', $olType2, '}.')"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:if>
  </xsl:template>
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <fo:block xsl:use-attribute-sets="ol-li-label">
            <xsl:choose>
              <xsl:when test="number($olStart) ge 0">
                <xsl:variable name="liSiblings"
                              select="../*[contains(@class,' topic/li ')]"/>

                <xsl:number format="{$format}" 
                  value="number($olStart) + u:indexOfNode($liSiblings, .) - 1"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:number format="{$format}"/>
              </xsl:otherwise>
  ...
  <xsl:template match="*[contains(@class,' topic/object ')]">
    <xsl:choose>
      <xsl:when 
        test="$foProcessor eq 'XEP' and 
              ($useMultimediaExtensions = 'yes' or
               exists(index-of($useMultimediaExtensions, string(@type))))">
        <xsl:call-template name="xepMediaObject"/>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:attribute-set name="xep-media-object">
    <xsl:attribute name="embed">true</xsl:attribute>
  </xsl:attribute-set>

  <xsl:template name="xepMediaObject">
    <!-- Very few content types seem to be supported. (Just .swf?) -->
    <rx:media-object src="{concat('url(', @data, ')')}" 
                     content-type="{concat('content-type:', @type)}"
                     xsl:use-attribute-sets="xep-media-object"
                     xmlns:rx="http://www.renderx.com/XSL/Extensions">
      <xsl:copy-of select="@id"/>

      <xsl:variable name="isAudio" select="starts-with(@type, 'audio/')"/>
      <xsl:if test="not($isAudio)">
        <!-- Supported units: px, pc, pt, cm, mm, in. -->
      ...
      <xsl:variable name="params"
                    select="./*[contains(@class,' topic/param ')]"/>
      <xsl:for-each select="$params">
        <xsl:variable name="paramName" select="string(@name)"/>
        <xsl:variable name="paramValue" select="string(@value)"/>

        <xsl:choose>
          <xsl:when test="not($isAudio) and
                          $paramName eq 'width' and number($paramValue) gt 0">
            <xsl:attribute name="content-width" 
                           select="concat($paramValue, 'px')"/>
          </xsl:when>
          <xsl:when test="not($isAudio) and
                          $paramName eq 'height' and number($paramValue) gt 0">
            <xsl:attribute name="content-height" 
                           select="concat($paramValue, 'px')"/>
          </xsl:when>
          <xsl:when test="$paramName eq 'loop'">
            <xsl:attribute name="play-mode" select="'continuously'"/>
          </xsl:when>
          <xsl:when test="$paramName eq 'muted'">
            <xsl:attribute name="volume" select="'silent'"/>
          </xsl:when>
          <xsl:when test="$paramName eq 'controls'">
            <xsl:attribute name="show-controls" select="'true'"/>
          </xsl:when>
          <xsl:when test="$paramName eq 'poster' and $paramValue ne ''">
            <xsl:attribute name="poster" 
                           select="concat('url(', $paramValue, ')')"/>
          </xsl:when>
        </xsl:choose>
      </xsl:for-each>

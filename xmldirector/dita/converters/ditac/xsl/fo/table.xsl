...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="columnWidth">
    ...
    <xsl:choose>
      <xsl:when test="$units eq 'pi'">pc</xsl:when>
      <xsl:when test="$units eq '' and $width ne ''">pt</xsl:when>
  ...
  <xsl:attribute-set name="thead" use-attribute-sets="table-header-style">
  </xsl:attribute-set>
    ...
    <xsl:choose>
      <xsl:when test="$isHeader">
        <fo:table-cell start-indent="0"
                       xsl:use-attribute-sets="entry table-header-style">

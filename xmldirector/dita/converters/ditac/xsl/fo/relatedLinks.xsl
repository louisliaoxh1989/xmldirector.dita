...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="showLinkInfo">
    ...
    <xsl:if test="($show-xref-page eq 'yes' and 
                   self::*[contains(@class,' topic/xref ')]) or 
                  ($show-link-page eq 'yes' and 
  ...
  <xsl:template name="showExternalLink">
    ...
    <xsl:if test="$show-external-links eq 'yes' and
                  (@scope eq 'external' or contains($href, '://'))">
  ...
  <xsl:template name="showLinkPage">
    ...
    <xsl:if test="$id ne ''">
      <xsl:variable name="pageRefBefore" 
        select="if ($page-ref-before eq '' and $page-ref-after eq '') 
                then u:localize('page-ref-before', .)
                else $page-ref-before"/>

      <xsl:variable name="pageRefAfter"
        select="if ($page-ref-before eq '' and $page-ref-after eq '') 
                then u:localize('page-ref-after', .)
                else $page-ref-after"/>

      <fo:inline xsl:use-attribute-sets="page-ref">
        <xsl:value-of select="$pageRefBefore"/>
        <fo:page-number-citation ref-id="{$id}"/>
        <xsl:value-of select="$pageRefAfter"/>
      </fo:inline>
    </xsl:if>
  </xsl:template>

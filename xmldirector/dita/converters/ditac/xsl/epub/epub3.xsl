...
<!--
| Copyright (c) 2013-2016 Pixware SARL. All rights reserved.
|
    ...
    <opf:manifest>
      ...
      <xsl:for-each select="$ditacLists/ditac:chunkList/ditac:chunk">
        ...
        <xsl:variable name="titlePage"
                      select="if (exists($chunk//ditac:titlePage))
                              then $ditacLists/ditac:titlePage
                              else ()"/>

        <xsl:variable name="properties">
          <!-- SVG embedded by inclusion or by reference. -->
          <xsl:if 
            test="exists($titlePage//svg:svg) or
                  exists($titlePage//*[contains(@class,' topic/image ') and
                                       (ends-with(@href, '.svg') or
                                        ends-with(@href, '.svgz'))]) or
                  exists($chunk//svg:svg) or
                  exists($chunk//*[contains(@class,' topic/image ') and
                                   (ends-with(@href, '.svg') or
                                    ends-with(@href, '.svgz'))])">svg</xsl:if>

          <xsl:if test="exists($titlePage//mml:math) or 
                        exists($chunk//mml:math)">

        ...
        <xsl:choose>
          <xsl:when test="@change-bar-placement eq 'start' or 
            ...
            <xsl:if test="exists(@change-bar-color)">
              border-left-color: <xsl:value-of select="@change-bar-color"/>;
            </xsl:if>
            border-left-width:
            <xsl:value-of select="if (exists(@change-bar-width)) 
                                  then @change-bar-width
                                  else 'medium'"/>;
            border-left-style: 
            <xsl:value-of select="if (exists(@change-bar-style)) 
          ...
          <xsl:otherwise>
            <xsl:if test="exists(@change-bar-color)">
              border-right-color: <xsl:value-of select="@change-bar-color"/>;
            </xsl:if>
            border-right-width:
            <xsl:value-of select="if (exists(@change-bar-width)) 
                                  then @change-bar-width

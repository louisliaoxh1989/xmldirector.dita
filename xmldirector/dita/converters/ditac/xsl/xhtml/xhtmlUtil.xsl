...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="titleAttribute">
    <xsl:if test="@ditac:title">
      <xsl:attribute name="title" select="string(@ditac:title)"/>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="otherAttributesImpl">
    ...
    <xsl:variable name="classes">
      <xsl:choose>
        <xsl:when test="$extraClasses ne ''">
          <xsl:value-of select="concat($baseClass, ' ', $extraClasses)"/>
        </xsl:when>
        <xsl:otherwise>
    ...
    <!-- Discard ol and table elements pseudo output classes. -->

    <xsl:variable name="outputclass">
      <xsl:choose>
        <xsl:when test="exists(@outputclass)">
          <xsl:variable name="classList">
            <xsl:sequence select="for $token in tokenize(@outputclass, '\s+')
                                  return 
                                      if (not($token eq 'lower-alpha' or 
                                              $token eq 'upper-alpha' or 
                                              $token eq 'lower-roman' or 
                                              $token eq 'upper-roman' or 
                                              $token eq 'decimal' or 
                                              $token eq 'continue' or 
                                              starts-with($token, 'start(') or
                                              starts-with($token, 'bgcolor(')))
                                      then $token
                                      else ()" />
          </xsl:variable>
          <xsl:value-of select="string-join($classList, ' ')"/>
        </xsl:when>
    ...
    <xsl:variable name="classValue">
      <xsl:choose>
        <!-- JavaHelp does not support multiple classes. -->
        <xsl:when test="$xhtmlVersion eq '-3.2'">
          <xsl:value-of select="$baseClass"/>
        </xsl:when>

        <xsl:when test="$outputclass ne ''">
          <xsl:value-of select="concat($classes, ' ', $outputclass)"/>
        </xsl:when>
  ...
  <!-- outputclass=bgcolor(XXX) support ================================== -->

  <xsl:template name="bgcolorStyle">
    <xsl:variable name="bgcolor"
      select="normalize-space(substring-before(substring-after(@outputclass, 
                                                               'bgcolor('), 
                                               ')'))"/>

    <xsl:sequence select="if ($bgcolor ne '') 
                          then concat('background-color: ', $bgcolor, ';') 
                          else ''"/>
  </xsl:template>

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:function name="u:hasChangebar" as="xs:boolean">
    <xsl:param name="flags" as="element()"/>

    <xsl:sequence select="exists($flags/@change-bar-color) or 
                          exists($flags/@change-bar-offset) or 
                          exists($flags/@change-bar-placement) or 
                          exists($flags/@change-bar-style) or 
                          exists($flags/@change-bar-width)" />
  </xsl:function>
